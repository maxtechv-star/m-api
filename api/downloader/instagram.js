...
module.exports = {
  meta: {
    ...
    params: [
      {
        name: 'url',
        desc: 'Instagram post/reel URL',
        example: 'https://www.instagram.com/p/DPWFQ5Hky1O/',
        required: true
      }
    ]
  },
  ...
  async onStart({ req, res }) {
    try {
      ...
      // Make request to Inflact
      const response = await axios.post(
        'https://inflact.com/downloader/api/downloader/post/',
        form,
        { headers: formHeaders, timeout: 30000 }
      );

      if (!response.data || !response.data.data) {
        throw new Error('Invalid response from Instagram service');
      }
      ...
      if (post.edge_sidecar_to_children && post.edge_sidecar_to_children.edges) {
        ...
        media_urls = post.edge_sidecar_to_children.edges.map(edge => edge?.node?.url || edge?.node?.display_url);
      } else {
        // Single media
        media_urls = [post.display_url || post.url];
      }
      ...
      const result = {
        ...
        comment_count: post.edge_media_to_parent_comment?.count || 0,
        is_video: post.is_video || false,
        media_count: media_urls.length,
        media_urls: media_urls.filter(url => url), // Remove empty/null URLs
      ...
      res.json({
        status: false,
        message: error.response?.data?.message || error.message || 'Failed to fetch Instagram post'
      });
    }
  }
};
